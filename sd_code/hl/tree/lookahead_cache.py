        ...
        for i, t in enumerate(token_ids):
            ...
            if tree is not None:
                ...
                if s >= branch_length // 2:
                    break
        # 处理 decoding_ids 为空的情况
        if decoding_ids is None or (isinstance(decoding_ids, torch.Tensor) and decoding_ids.numel() == 0):
           decoding_ids = token_ids[-1:]

       # 确保 sizes 不为空
        if sizes is None:
           sizes = [0, 0]
         # 如果 decoding_ids 仍然是一个张量，可以通过 .item() 取出单一值，避免布尔运算时的歧义
        if isinstance(decoding_ids, torch.Tensor) and decoding_ids.numel() == 1:
            decoding_ids = decoding_ids.item()  # 取出张量的单一值   
